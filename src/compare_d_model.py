...
# ===================== 字体配置（简化为英文，避免中文问题） =====================
def setup_font():
    # 直接使用默认英文字体，彻底避免中文兼容性问题
    plt.rcParams['font.family'] = ['DejaVu Sans', 'sans-serif']
    plt.rcParams['axes.unicode_minus'] = False
    print(" 字体配置完成（使用英文默认字体）")
...
# ===================== 简化超参配置（固定num_heads=4，仅测试3个d_model） =====================
TEST_D_MODELS = [256, 384, 512]  # 仅测试这3个模型维度
#TEST_D_MODELS = [256]
FIXED_PARAMS = {
    "num_heads": 4,  # 固定注意力头数为4
    "batch_size": 128,
    "max_len": 64,
    "num_workers": 0,
...
RESULT_DIR = os.path.join(PROJECT_ROOT, "results", "d_model_comparison")
...
# ===================== 单组训练（返回每轮loss和BLEU，用于后续对比） =====================
def train_single_d_model(d_model):
    params = FIXED_PARAMS.copy()
    params["d_model"] = d_model
    params["d_ff"] = params["d_ff"](d_model)
    set_seed(params["seed"])
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    print(f"\n{'=' * 60}")
    print(f"当前训练：d_model={d_model}, num_heads={params['num_heads']}（固定）, dropout={params['dropout']}")
    print(f"{'=' * 60}")
...
# ===================== 主函数（简化：训练3个d_model + 生成2个对比曲线） =====================
def main():
    print(f"开始测试d_model组合：{TEST_D_MODELS}（num_heads固定为4）")
    print(f"固定dropout值：{FIXED_PARAMS['dropout']}")

    # 训练所有d_model，收集指标
    all_results = []
    for d_model in TEST_D_MODELS:
        result = train_single_d_model(d_model)
        all_results.append(result)

    # 生成并保存【val_loss对比曲线】（按epoch变化）
    ...
    plt.title('Validation Loss Comparison of Different Model Dimensions (num_heads=4, dropout=0.1)')  # 英文替换
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    val_loss_path = os.path.join(RESULT_DIR, "val_loss_comparison.pdf")
    plt.savefig(val_loss_path, bbox_inches='tight', dpi=None)
    plt.close()
    print(f"\n 验证损失对比曲线已保存：{val_loss_path}")

    # 生成并保存【BLEU对比曲线】（按epoch变化）
    plt.figure(figsize=(10, 6))
    for result in all_results:
        d_model = result["d_model"]
        plt.plot(
    ...
    plt.title('BLEU Score Comparison of Different Model Dimensions (num_heads=4, dropout=0.1)')  # 英文替换
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    bleu_path = os.path.join(RESULT_DIR, "bleu_comparison.pdf")
    plt.savefig(bleu_path, bbox_inches='tight', dpi=None)
    plt.close()
    print(f" BLEU分数对比曲线已保存：{bleu_path}")

    print(f"\n{'=' * 60}")
    print("所有d_model训练完成！已生成2个对比曲线PDF文件。")
    print(f"文件路径：{RESULT_DIR}")
    print(f"{'=' * 60}")
